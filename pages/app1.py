...
def load_sentiment_resources():
    try:
        model = load_model('best_modelQ2.keras')
        with open('tokenizerQ2a.pkl', 'rb') as handle:
            tokenizer = pickle.load(handle)
        return model, tokenizer
    except FileNotFoundError:
        st.error("Pastikan file 'best_modelQ2.keras' dan 'tokenizerQ2a.pkl' tersedia.")
        st.stop()

model, tokenizer = load_sentiment_resources()

# --- Antarmuka Aplikasi Streamlit ---
st.title("Analisis Sentimen Cagub 2025 Provinsi Bengkulu Paslon 02")
